...
if [ -f "src/MarketDelegation.sol" ]; then
    echo -e "${GREEN}✅ MarketDelegation.sol found${NC}"
    echo "   Location: src/MarketDelegation.sol"
    echo "   Size: $(wc -l < src/MarketDelegation.sol) lines"
else
...
if [ -f "test/MarketDelegation.t.sol" ]; then
    ...
    echo -e "${YELLOW}⚠️  MarketDelegation.t.sol not present; continuing with the repository test suite${NC}"
fi
echo ""

# Try to compile
echo "5. Compiling MarketDelegation.sol..."
if forge build --contracts src/MarketDelegation.sol 2>&1 | grep -q "Compiler run successful"; then
