...
contract MarketInsurance {
    ...
    function createPolicy(
        ...
        if (!(durationDays >= MIN_COVERAGE_DURATION && durationDays <= MAX_COVERAGE_DURATION)) {
            revert InvalidPolicyDuration();
        }

        // Calculate premium
        uint256 premium = calculatePremium(coverageAmount, durationDays, tier);
        if (!(premium > 0)) revert InvalidPremiumRate();
    ...
    function renewPolicy(uint256 policyId, uint256 newDurationDays) external {
        ...
        if (!(newDurationDays >= MIN_COVERAGE_DURATION && newDurationDays <= MAX_COVERAGE_DURATION)) {
            revert InvalidPolicyDuration();
        }
    ...
    function updateCoverageUtilization(
        ...
        if (!(utilizationAmount <= coverage.coveredAmount)) revert InvalidCoverage();
    ...
    function submitClaim(
        ...
        InsurancePolicy storage policy = _policies[policyId];
        if (!(policy.policyHolder == msg.sender)) revert UnauthorizedClaim();
        if (!(policy.status == PolicyStatus.ACTIVE)) revert PolicyNotActive();
        require(policy.expiryTime > block.timestamp, "Policy expired");
        if (!(claimAmount > 0)) revert InvalidClaimAmount();
        if (!(claimAmount <= policy.coverageAmount)) revert InvalidClaimAmount();
    ...
    function approveClaim(uint256 claimId, uint256 approvedAmount) external {
        ...
        if (!(approvedAmount <= insuranceFundBalance)) revert InsufficientFunds();
    ...
    function payClaim(uint256 claimId) external {
        ...
        if (!(block.timestamp <= claim.processedTime + CLAIM_PROCESSING_PERIOD)) {
            revert ClaimExpired();
        }
    ...
    function getClaim(
        ...
        if (!(claimId < _nextClaimId)) revert ClaimNotFound();
    ...
    function updatePremiumRates(
        ...
        if (!(msg.sender == admin)) revert Unauthorized();
        if (!(basicRate > 0 && standardRate > 0 && premiumRate > 0 && platinumRate > 0)) {
            revert InvalidPremiumRate();
        }
    ...
    function withdrawInsuranceFund(uint256 amount) external {
        if (!(msg.sender == admin)) revert Unauthorized();
        require(amount > 0, "Amount must be greater than 0");
        if (!(amount <= insuranceFundBalance)) revert InsufficientFunds();
    ...
    function getPolicy(
        ...
        if (!(policyId < _nextPolicyId)) revert InvalidPolicy();
