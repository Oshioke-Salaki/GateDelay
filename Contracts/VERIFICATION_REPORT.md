...
The same scoped commands are executed by `.github/workflows/forge-tests.yml` for the `Contracts/` suite. The workflow sets `FOUNDRY_TEST=test/marketfactory`, whose wrapper imports the canonical `test/MarketFactory.t.sol`, so unrelated legacy tests are not compiled for this phase-gated verification. The CI profile uses 512 fuzz runs.
